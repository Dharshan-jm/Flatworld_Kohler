...
public class TC_03_VerifyPointsOnHoldUnLockTest extends BaseClass{
	
	@Test
	public void verifyPointsOnHoldUnLock3() throws InterruptedException, EncryptedDocumentException, IOException {
		...
		publicuserpage.getTaxcheckbox().click();
		...
		trainingpage.getRequiredcheckbox().click();
		...
		if(navigatetitlepage.getQuizPassingMark().isDisplayed())
		{
			if(navigatetitlepage.getQuizPassingMark().isEnabled())
			{
				...
				navigatetitlepage.getQuizSuccessMessage().sendKeys("added");
		...
		Reporter.log("----NAVIGATE TO USER ACCOUNT----", true);
		...
		try {
		transactionpage.getLock().isDisplayed();
		} catch (Exception e) {
			Reporter.log("-----LOCK IS OPEN-----", true);
		}    
		Reporter.log("-----TEST COMPLETED-----", true);		
		
		assertionUtility.assertTrue(!webDriverUtility.checkElementIsDisplayed(transactionpage.getLock()), "LOCK IS OPEN");
	}
