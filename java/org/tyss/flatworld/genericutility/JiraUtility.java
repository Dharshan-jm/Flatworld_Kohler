...
public class JiraUtility {
	 ...
	 */
	public void addTestCaseToCycleAndUpdateResults(String testCaseName, String cycleId, String testCaseId, String status) {
		// Creating the JSON request body
		JSONObject requestParams = new JSONObject();
		requestParams.put("projectKey", PROJECT_KEY);
	...
	public String checkIfTestCaseExists(String testCaseName) {
		...
		if (result) {
			...
			for (int i = 0; i < tcs; i++) {
				if (JsonPath.read(response.asString(), "$.values[" + i + "].name").equals(testCaseName)) {
